...
namespace HyperMsg.Transport
{
    ...
    {
        private WorkerDataHandler worker;
        ...
        public async Task OpenTransport_Periodically_Invokes_AsyncAction()
        {
            ...
            worker = new WorkerDataHandler(async t =>
            {
                if (invokeCount == 0)
                {
        ...
        public async Task ClosedTransport_Stops_Invoking_AsyncAction_And_Invokes_BackgroundTaskCompleted()
        {
            ...
            worker = new WorkerDataHandler(t =>
            {
                @event.Wait();
                wasInvoked = true;
        ...
        public async Task Invokes_UnhandledException_When_AsyncAction_Throws_Exception()
        {
            ...
            worker = new WorkerDataHandler(t => throw expected);
        ...
        public async Task Dispose_Initiates_Background_Task_Cancellation()
        {
            ...
            worker = new WorkerDataHandler(t => Task.Delay(waitTimeout));
        ...
        private Task RunWorkerAsync(CancellationToken cancellationToken = default) => worker.HandleTransportEventAsync(TransportEvent.Opened, cancellationToken);

        private Task StopWorkerAsync(CancellationToken cancellationToken = default) => worker.HandleTransportEventAsync(TransportEvent.Closed, cancellationToken);
